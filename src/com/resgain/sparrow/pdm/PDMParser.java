...
public class PDMParser
{
    ...
    public static Project parse(String pdmContent)
    {
        ...
        while (lMatcher.find()) {
        	String comment = getString(lMatcher.group(11), "<a:Comment>(?s)(.*?)</a:Comment>", 1, null);
            Table table = new Table(ret.getCode(), lMatcher.group(2), lMatcher.group(9), lMatcher.group(5), comment);
...
//						col.setScode(col.getCode().substring((prefix1.length()+1)));
//						try {
//							(prefix1.length()+1) < col.getCode().length() ? 
//									
//							col.setScode(col.getCode().substring((prefix1.length()+1) < col.getCode().length() ? );
//						} catch(Exception e) {
//							System.out.println(prefix1.length()+1);
//							System.out.println(col.getCode());
//							System.exit(0);
//							
//							
//						}
